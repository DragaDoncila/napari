...
class _Coordinates(Layer):
    ...
    _drag_modes: ClassVar[
        dict[Mode, Callable[['_Coordinates', Event], Any]]
    ] = {
        Mode.PAN_ZOOM: no_op,
    ...
    def __init__(
        ...
        *,
        ...
        self._coordinates = np.asarray(data)
    ...
    @coordinates.setter
    def coordinates(self, data: Optional[np.ndarray]) -> None:
        """Set the data array and emit a corresponding event."""
        prior_data = len(self.coordinates) > 0
        data_not_empty = (
            data is not None
        ...
        kwargs = {
            ...
            'vertex_indices': ((),),
            'data_indices': tuple(i for i in range(len(self.coordinates))),
        }
        ...
        self.events.coordinates(**kwargs)
        self._set_coordinates(data)
        kwargs['data_indices'] = tuple(i for i in range(len(self.coordinates)))
        kwargs['value'] = self.coordinates
    ...
    def features(
        ...
        self._feature_table.set_values(
            features, num_data=len(self.coordinates)
        )
    ...
    def _extent_data(self) -> np.ndarray:
        ...
        if len(self.coordinates) == 0:
            ...
            maxs = np.max(self.coordinates, axis=0)
            mins = np.min(self.coordinates, axis=0)
    ...
    def symbol(self, symbol: Union[str, np.ndarray, list]) -> None:
        symbol = np.broadcast_to(symbol, self.coordinates.shape[0])
    ...
    def shown(self, shown):
        self._shown = np.broadcast_to(shown, self.coordinates.shape[0]).astype(
            bool
        )
        self.refresh()
    ...
    def border_width(
        ...
        border_width = np.broadcast_to(
            border_width, self.coordinates.shape[0]
        ).copy()
    ...
    def add(self, coords):
        ...
        """
        cur_points = len(self.coordinates)
        ...
        self._set_coordinates(
            np.append(self.coordinates, np.atleast_2d(coords), axis=0)
        )
        self.events.coordinates(
            value=self.coordinates,
        ...
        self.selected_data = set(np.arange(cur_points, len(self.coordinates)))
    ...
    def remove_selected(self) -> None:
        ...
        if len(index):
            ...
            self._set_coordinates(np.delete(self.coordinates, index, axis=0))
            self.events.coordinates(
                value=self.coordinates,
    ...
    def _move(
        ...
        if len(selection_indices) > 0:
            ...
            center = self.coordinates[np.ix_(selection_indices, disp)].mean(
                axis=0
            )
            shift = np.array(position)[disp] - center - self._drag_start
            self.coordinates[np.ix_(selection_indices, disp)] = (
                self.coordinates[np.ix_(selection_indices, disp)] + shift
            )
            self.refresh()
            self.events.coordinates(
                value=self.coordinates,
    ...
    def _paste_data(self) -> None:
        ...
        totpoints = len(self.coordinates)
        ...
        if len(self._clipboard.keys()) > 0:
            ...
            self._coordinates = np.append(self.coordinates, data, axis=0)
    ...
    def _copy_data(self) -> None:
        ...
        if len(self.selected_data) > 0:
            index = list(self.selected_data)
            self._clipboard = {
                'data': deepcopy(self.coordinates[index]),
    ...
    def to_mask(
        ...
        *,
        ...
        points_in_mask_data_coords = np.atleast_2d(
            points_data_to_mask_data(self.coordinates)
        )
    ...
    def _get_properties(
        ...
        *,
        ...
        if value is None or value > self.coordinates.shape[0]:
            return []
